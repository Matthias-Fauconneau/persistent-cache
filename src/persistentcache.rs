...
macro_rules! cache_func {
    ...
    (Redis, $host:expr, fn $f:ident($($x:ident : $t:ty),*) -> $r:ty $b:block) => {
        cache_func!(Redis, $host, "", fn $f($($x : $t),*) -> $r $b);
    };
    // Create `FileStorage` with default prefix
    (File, $dir:expr, $(#[$attr:meta])* $v:vis fn $f:ident($($x:ident : $t:ty),*) -> $r:ty $b:block) => {
        $(#[$attr])* $v fn $f($($x: $t),*) -> $r {
            $crate::persistentcache::lazy_static::lazy_static!{
                // In order to be consistent with `RedisStorage`, `FileStorage` also uses a Mutex.
                // However, it would not be necessary.
                static ref S: std::sync::Mutex<$crate::storage::file::FileStorage> = std::sync::Mutex::new($crate::storage::file::FileStorage::new($dir).unwrap());
            };
            $crate::cache_func!($f($($x),*), $b, "");
        }
    };
    ...
    ($f:ident($($x:ident),*), $b:block, $prefix:expr) => {
        ...
        let var_name = format!("{}_{}_{}_{:?}", $crate::PREFIX, $prefix, stringify!($f), s.finish());
        use persistentcache::PersistentCache;
        ...
        match result.len() {
            ...
            _ => return $crate::persistentcache::bincode::deserialize(&result).unwrap(),
        }
    }
}
    ...
    ($storage:ident, $func:ident($($x:expr),*), $prefix:expr) => {
        (||{
            ...
            match result.len() {
                0 => {
                    res = $func($($x),*);
                    $storage.set(&var_name, &$crate::persistentcache::bincode::serialize(&res).unwrap()).unwrap();
                    res
                    // match $func($($x),*) {
                    //     Ok(res) => {
                    //         $storage.set(&var_name, &$crate::persistentcache::bincode::serialize(&res)?)?;
                    //         Ok(res)
                    //     }
                    //     Err(e) => Err(e)
                    // }
                },
                // _ => match $crate::persistentcache::bincode::deserialize(&result) {
                //     Ok(res) => res,
                //     Err(e) => panic!(e.into()), // I have no idea what I am doing.
                // }
                _ => {
                    res = $crate::persistentcache::bincode::deserialize(&result).unwrap();
                    res
                }
            }
       })()
